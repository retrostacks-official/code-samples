...
use App\Models\VisitsHasStatus;
...
use Carbon\Carbon;
use App\Gateways\CscartGateway;

use function GuzzleHttp\json_decode;


/**
 * Class To manage antibiotics
 * */
...
class MedicalReportsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->ajax())
        {
            $model=Group::query()->whereHas('visit', function($q) {
                $q->where('status', '!=','16');
            })
                    ->with('patient','tests','cultures','contract','signed_by_user','created_by_user','all_barcodes')
            ...
            {
                $model->where('all_barcodes.barcode',$request['filter_barcode']);
            }
            ...
            ->editColumn('code', function ($group) {
                $profile_name = ($group['visit']['PatientProfiles'] == null)? '' : $group['visit']['PatientProfiles']['name'];
                return $profile_name;
                
            })
            ->editColumn('phone', function ($group) {
                $phone = ($group['visit']['PatientProfiles'] == null)? '' : $group['visit']['PatientProfiles']['phone'];
                return $phone;
                
            })
            ->editColumn('all_barcodes',function($group){
                return view('admin.medical_reports._barcodes',compact('group'));
            })
    ...
    public function rejectreport(Request $request)
    {
        ...
        return view('admin.medical_reports.index_reject');
    }
    ...
    public function requestreport(Request $request)
    {
        ...
        {
            $model=Group::query()
                    ->with('patient','tests','cultures','contract','signed_by_user','created_by_user','all_barcodes','visit.PatientProfiles')
                    ->where('branch_id',session('branch_id'))
                    ->where('accepted',0);
            ...
            ->editColumn('phone', function ($group) {
                $profile_name = ($group['visit']['PatientProfiles'] == null)? '' : $group['visit']['PatientProfiles']['phone'];
                return $profile_name;
        ...
        return view('admin.medical_reports.requests'); 
    }
    ...
    public function show($id)
    {
        ...
        $group=Group::with(['patient','patient.country'])->where('branch_id',session('branch_id'))
                    ->findOrFail($id);
                    // return $group;
    ...
    public function pdf(Request $request,$id)
    {
        ...
        $group=Group::with('visit')->findOrFail($id);
    ...
    public function edit($id)
    {
        ...
        $branches = Branch::where('id', '!=', session('branch_id'))->where('id', '!=', 1)->get();
        $phlebo_users = DB::table('users')->select('users.id','users.name')
                    ->join('user_branches', 'user_branches.user_id', '=', 'users.id')
                    ->join('user_roles', 'user_roles.user_id', '=', 'users.id')
                    ->where('user_branches.branch_id',session('branch_id'))
                    ->where('user_roles.role_id',3)->get();
        
        $group=Group::with(['TransferReports','visit','visit.visit_status','visit.PatientProfiles','visit.PatientProfiles.relationship','all_tests'=>function($q){
        ...
        return view('admin.medical_reports.edit',compact('group','select_antibiotics','next','previous','branches','phlebo_users'));
    }
    ...
    public function accept($id)
    {
         ...
         $group->update([
            'accepted'=>auth()->guard('admin')->user()->id
        ]);
        session()->flash('success',__('Accepted'));
    ...
    public function upload_report(UploadReportRequest $request,$id)
    {
        ...
        if($request->has('report'))
        {
            ...
            $report->move('public/uploads/pdf','report_'.$group['id'].'.pdf');
            
            $group->update([
                'uploaded_report'=>true,
                'done'=>true,
                'report_pdf'=>url('public/uploads/pdf/report_'.$group['id'].'.pdf')
            ]);
            ...
            $datas = array (
                'fake' => '1',
                'selected_section' => 'order_oi_attachments',
                'object_id' => $group['visit']['cscart_id'],
                'object_type' => 'order',
                'attachment_id' => '0',
                'redirect_url' => $URL.'?dispatch=orders.details&order_id='.$group['visit']['cscart_id'],
                'attachment_data' => 
                array (
                'description' => 'Result',
                'position' => '',
                ...
                'file_attachment_files' => 
                array (
                0 =>  url('public/uploads/pdf/report_'.$group['id'].'.pdf'),
                ),
                'type_attachment_files' => 
                array (
                0 => 'url',
                ),
                'notifi_about_new_file' => 'N',
                'dispatch' => 'attachments.update',
                'security_hash' => 'f8ca076fcd08f631abf8156b379da9ea',
            );
            ...
            $cscartgateway = new CscartGateway();
            //cs cart api for create visit starts
            $cscart_id = $cscartgateway->sendResult($datas);
            //cs cart api for create visit ends
            //cs cart api ends
    ...
    //custom func to  transfer sample from one lab to another
    public function transfer_report(Request $request, $id){
        ...
        $transfer=new TransferReports;
        $transfer->group_id=$request->id;
        $transfer->visit_id=$request->visit_id;
        $transfer->transfer_from=session('branch_id');
        $transfer->transfer_to=$request->branch_id;
        $transfer->created_at=date('d-m-Y H:i:s');
        $transfer->updated_at=date('d-m-Y H:i:s');
        ...
        session()->flash('success',__('Transfer Request send to successfully'));
    ...
    //command center reassign to phlebotomists

    public function reject_reasign(Request $request, $id){
        
        $data = $request->all();
        $data = json_encode($data);
        $result = json_decode($data);
        unset($result->phlebo_id);
        unset($result->reassign);
        ...
        Visit::where('id',$request->visit_id)->update(['re_assign'=>true,'user_id'=>$phlebo_id,'status'=>17]);
        $data = array(
            'order_id'=>$request->visit_id,
            'status_id'=>17,
    ...
    VisitsHasStatus::insert($data);
    VisitsHasStatus::insert($data1);



        session()->flash('success',__('Re assign successfully'));
    ...
    /**
     * Reject sample if sample has any issues
     *
     * 
     */
    public function reject_report(Request $request, $groupid){
        

        // $result = json_decode($request);
        $result = json_encode($request->all());
        $result = json_decode($result);
        unset($result->visit_id);
       
        // Group::where('id',$groupid)->update(['sample_rejected'=>true]);
        Visit::where('id',$request->visit_id)->update(['re_assign'=>true,'status'=>16,'user_id'=>$request->phlebo_id]);
        $VisitsHasStatus = new VisitsHasStatus;
        $VisitsHasStatus->order_id = $request->visit_id;
        $VisitsHasStatus->status_id = 16;
        $VisitsHasStatus->comment = json_encode($result);
        $VisitsHasStatus->userid = session('user_id');
        $VisitsHasStatus->save();

        session()->flash('success',__('Report send to Command center'));
        return redirect()->back();
